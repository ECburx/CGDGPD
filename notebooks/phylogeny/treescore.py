...
def return_astral_score(astrolout, logfile):
    ...
    return astraldf, {'mean': astral_stas.mean, 'std': astral_stas.variance, 'skewness': astral_stas.skewness,
                      'nancount': nancount, 'duploss': duploss}


def run_astral(phylo, st, mapping, output, root, outfmt='2',
               astralpath='/work/FAC/FBM/DBC/cdessim2/default/dmoi/software/ASTER-Linux/bin/astral-pro3'):
    logfile = output + '.log'
    cmd = astralpath + ' -c {st} -a {mapping} -u {outfmt} -i {phylo} -o {output} -C -T -E --root {root} 2>{logfile}'.format(
        phylo=phylo, st=st, mapping=mapping, output=output, root=root, logfile=logfile, outfmt=outfmt)
    print(cmd)
...
def lineage_score_tax_degree(node, uniprot_df):
    ...
    # add the weighted score
    print('clades', clades)
    score = sum([sum([clades[l] * taxa_degree[l] for l in n.lineage]) for n in node.traverse() if
                 n.is_leaf() == False and n.lineage])
    print('lineage score', score)
    return score
